...
export async function createOrder(
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
        ...
        try {
            ...
            attemptLogger.info('BitMEX createOrder success after %d attempt(s) in %dms', attemptNumber, elapsedMs, {
...
export async function getOrderByClOrdId(
    ...
    try {
        ...
        requestLogger.error('BitMEX getOrder by clOrdID=%s failed after %dms: %s', clOrdID, elapsedMs, errorMessage, {
            clOrdID,
            elapsedMs,
            timeoutMs,
            errorName,
