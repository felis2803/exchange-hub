...
export class Position extends EventEmitter implements BaseEntity<PositionSnapshot> {
    ...
    override on(
        event: 'update',
        listener: (next: PositionSnapshot, diff: DomainUpdate<PositionSnapshot>, reason?: PositionUpdateReason) => void,
    ): this;
...
export class PositionsRegistry {
    ...
    #listeners = new WeakMap<
        Position,
        (snapshot: PositionSnapshot, diff: DomainUpdate<PositionSnapshot>, reason?: PositionUpdateReason) => void
    >();
    #onUpdate?: PositionUpdateListener;
    #view: PositionsView;
    ...
    #createView(): PositionsView {
        const view = {
            ...
            byAccount: accountId => this.#positionsIterator(this.#byAccount.get(normalizeAccountId(accountId)) ?? []),
            bySymbol: symbol => this.#positionsIterator(this.#bySymbol.get(normalizeSymbol(symbol)) ?? []),
