...
function mergeTags(base?: readonly unknown[], extra?: readonly unknown[]): readonly string[] | undefined {
    const normalizedBase = base?.filter((value): value is string => typeof value === 'string') ?? [];
    const normalizedExtra = extra?.filter((value): value is string => typeof value === 'string') ?? [];
