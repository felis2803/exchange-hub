...
describe('BitMEX position stream', () => {
    test('dedupe-out-of-order updates keep state consistent', () => {
        ...
        try {
            ...
            expect(hub.positions.activeArray().map(position => position.symbol)).toEqual(['XBTUSD', 'ETHUSD']);
            ...
            const expectedSymbols = ['XBTUSD', 'ETHUSD', 'XBTUSD', 'ETHUSD', 'XBTUSD', 'XBTUSD', 'ETHUSD', 'ADAUSD'];

            expect(incrementCounterSpy).toHaveBeenCalledTimes(expectedSymbols.length);
            expect(incrementCounterSpy.mock.calls.map(([, , labels]) => labels.symbol)).toEqual(expectedSymbols);
        } finally {
    ...
    test('reconnect-resync waits for partial before applying updates', () => {
        ...
        try {
            ...
            const expectedSymbols = ['XBTUSD', 'ETHUSD', 'XBTUSD', 'XBTUSD', 'SOLUSD', 'ETHUSD', 'SOLUSD', 'XBTUSD'];
