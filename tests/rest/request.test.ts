...
describe('BitmexRestClient.request()', () => {
    ...
    test('throws AuthError when auth requested without credentials', async () => {
        ...
        await expect(client.request('GET', '/api/v1/user/margin', { auth: true })).rejects.toBeInstanceOf(AuthError);
    ...
    test('maps 5xx to ExchangeDownError', async () => {
        ...
        await expect(client.request('GET', '/api/v1/instrument/active')).rejects.toBeInstanceOf(ExchangeDownError);
    });
});
