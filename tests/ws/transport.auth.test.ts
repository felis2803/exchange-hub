...
describe('BitmexWsClient auth flows', () => {
  ...
  test('clock skew error resolved by increasing expires skew', async () => {
    ...
    const lowSkewError = new Promise<AuthError>((resolve) =>
      lowSkewClient.once('auth_error', resolve),
    );
    ...
    const highSkewAuthed = new Promise<{ ts: number }>((resolve) =>
      highSkewClient.once('authed', resolve),
    );
