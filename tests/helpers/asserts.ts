...
export function expectNoChanges<T extends Record<string, unknown>>(diff: DomainUpdate<T> | null | undefined): void {
    if (!diff) {
        return;
    }
...
export function expectHistogramIncludes(name: string, expected: number, labels?: MetricLabels, tolerance = 1e-6): void {
    const values = getHistogramValues(name, labels);

    expect(values.some(value => Math.abs(value - expected) <= tolerance)).toBe(true);
}

export function expectHistogramValues(name: string, expected: readonly number[], labels?: MetricLabels): void {
